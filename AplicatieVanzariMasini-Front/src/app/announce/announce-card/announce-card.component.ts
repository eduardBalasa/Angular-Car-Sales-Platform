...
export class AnnounceCardComponent implements OnInit {
  ...
  saveAnnounce(id: number) {
    this.announceService
      .saveAnnounce(this.authService.decodedToken.nameid, id)
      .subscribe(
        (data) => {
  ...
  deleteAnnounce() {
    this.alertify.confirm("Esti sigur ca vrei sa stergi acest anunt?", () => {
      this.announceService.deleteAnnounce(this.announce).subscribe(
        (data) => {
          this.alertify.success("Ai sters anuntul: " + this.announce.title);
        },
        (error) => {
          this.alertify.error(error);
        }
